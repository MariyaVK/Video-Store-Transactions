...
public class VideoStore {
	...
	/**
	 * Updates the splay tree inventory by adding a number of video copies of
	 * the film. (Splaying is justified as new videos are more likely to be
	 * rented.)
	 * 
	 ...
	 * been added. b) If false is returned, the film is already on the
	 * inventory.
	 * 
	 * The root of the splay tree must store the corresponding Video object for
	 * the film. Update the number of copies for the film.
	 * 
	 * @param film
	 *            title of the film
	 * @param n
	...
	public void addVideo(String film, int n) {
		...
		boolean added = inventory.add(video);
		if (!added)
	 ...
	 *             if n <= 0 or film == null or film.isEmpty()
	...
	public void videoRent(String film, int n)
		...
		if (n <= 0 || film == null)
			...
			else if (video.getNumAvailableCopies() <= 0)
	 ...
	 * or is rented out, an exception will be thrown from videoRent(). Based on
	 * the exception, prints out the following message: "Film <film> is not in
	 * inventory" or "Film <film> has been rented out." In the message, <film>
	 * shall be replaced with the name of the video. b) Otherwise, update the
	 * video record in the inventory.
	 * 
	 * For details on handling of multiple exceptions and message printing,
	 * please read Section 3.4 of the project description.
	 * 
	 * @param videoFile
	 ...
	 * videoReturn() does. See Section 3.4 of the project description on how to
	...
	/**
	 * Performs inorder traversal on the splay tree inventory to list all the
	...
	public String inventoryList() {
		...
		while (iter.hasNext()) {
			...
			str += video.getFilm() + " (" + video.getNumCopies() + ")\n";
		}
		return str;
	}
	 ...
	 * Rented films:
	 * 
	 * Brokeback Mountain (1) Forrest Gump (1) Singin' in the Rain (2) The
	 * Godfather (1)
	 * 
	 * 
	 * @return
	...
	private String rentedVideosList() {
		...
		while (iter.hasNext()) {
			Video video = iter.next();
			if (video.getNumRentedCopies() > 0)
				str += video.getFilm() + " (" + video.getNumRentedCopies() + ")\n";
		}
		return str;
	}
	...
	private String unrentedVideosList() {
		...
		while (iter.hasNext()) {
			Video video = iter.next();
			if (video.getNumAvailableCopies() > 0)
				str += video.getFilm() + " (" + video.getNumAvailableCopies() + ")\n";
		}
		return str;
	}
